...
export function NovoCartao({ setEtapaCarteira, getCards }: any) {
  ...
  function sameBillingAddress(checked: boolean) {
    if (checked) {
      ...
      setValue("zip_code", firstAddress?.zip_code);
    ...
    >
      ...
      <HStack pt={6} maxW={1200} w="full" justify="flex-end">
        ...
        {errorCard && (
          <AlertInpa
            status="warning"
            text={errorCard?.response?.data?.error?.message || "Erro"}
          />
        )}
        <Button
          onClick={() => setEtapaCarteira("home")}
          color="white"
          textColor="cinza"
          title="Voltar"
        />
        <Button
          // onClick={() => setEtapaCarteira("pagamento")}
          type="submit"
          px={8}
          title="Adicionar"
